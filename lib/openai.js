...
export const getResults = async ({ term, searchResults }) => {
  const prompt = `
      ...
      Search only through \`Course name\` and \`Course description\`. Ignore \`Course id\` in search process.
      ...
      Return the relevant courses as a JSON array with the following structure: [{ "name": "string", "relevance": "float", "id": "integer" }].
